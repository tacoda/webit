...
class DomainManager:
    ...
    def find_hosted_zone(self, domain_name):
        """Find zone matching the domain name"""
    ...
    def create_hosted_zone(self, domain_name):
        """Create a hosted zone to match the domain name"""
    ...
    def create_s3_domain_record(self, zone, domain_name, endpoint):
        """Create a domain record in zone for the domain name"""
        return self.client.change_resource_record_sets(
            HostedZoneId=zone['Id'],
            ChangeBatch={
                'Comment': 'Created by WebIt',
                'Changes': [{
                    'Action': 'UPSERT',
                    'ResourceRecordSet': {
                        'Name': domain_name,
